...
/**
 * `GET /api/contact-export` — protected read of the stored contact messages.
 *
 * The messages file (`/data/contact.jsonl`) is owned by the container user and
 * is not readable from outside the app, so this endpoint is how the team
 * reads messages that email could not deliver. Requires a bearer token that matches CONTACT_EXPORT_TOKEN, falling back to
 * WAITLIST_EXPORT_TOKEN so the team can use the token they already hold.
 *
 *   curl -H "Authorization: Bearer <token>" https://nuroai.dev/api/contact-export
 *
...
export const GET: APIRoute = async ({ request, url }) => {
  const token =
    process.env.CONTACT_EXPORT_TOKEN || process.env.WAITLIST_EXPORT_TOKEN;
  if (!token) {
  ...
  try {
    ...
    // No file yet means nothing has been submitted.
